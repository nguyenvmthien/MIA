...
[Nguồn dữ liệu]
       ...
       └── 3. Feedback user  → PostgreSQL feedback tables → JSONL export
                  │
                  ▼
...
Mỗi lần user submit correction qua API, hệ thống lưu feedback vào PostgreSQL. JSONL chỉ là artifact export để train/evaluate.

```bash
# Export SFT data from DB-backed interactions
make export-sft

# Or call the API export endpoint when the API is running
curl "http://localhost:8000/feedback/export?format=jsonl" \
  -o data/training/feedback_corrections.jsonl
```
